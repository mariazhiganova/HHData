...
from src.api_connection import EmployerHH
...
def select_employers_ids():
    ...
    while len(employers_id) < 10:
        ...
        selected_ids = input("Введите номера работодателей для добавления в список (через запятую): ")
        ...
        for index in selected_indices:
            try:
                ...
                print(f"Некорректный ввод: {index}. Пожалуйста, введите номера через запятую.")

        print(f"Текущий список выбранных работодателей: {employers_id}")

    print("Вы достигли максимального количества работодателей.")
    return employers_id


def get_full_employers_info(employer_ids):
    """
    ...
    for employer_id in employer_ids:
        info_list = employers._connection(employer_id)  # Получаем список работодателей
        if info_list:  # Если список не пустой
            employer_info_list[employer_id] = info_list[0]  # Берем первый элемент списка
        else:
            employer_info_list[employer_id] = {}  # Если нет данных, добавляем пустой словарь

    return employer_info_list
